      ...
      <section className="pt-40 pb-12">
        <div className="mx-auto max-w-5xl px-6 text-center lg:px-12">
          ...
          <p className="mx-auto mt-5 max-w-xl text-forest-deep/75">
            Choose the powders, the packaging and the pace. We'll come back with samples and a quote
          ...
          <div className="rounded-3xl glass p-8 lg:p-10">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {selected.map((slug) => {
                  ...
                  return (
                    <span
                      key={slug}
                      className="inline-flex items-center gap-1.5 rounded-full bg-turmeric px-3 py-1 text-xs font-medium text-forest-deep"
                    >
                      {p.name}
                      <button
                        type="button"
            ...
            >
              <span>
                {selected.length === 0
                  ? "Select powders…"
                  : `${selected.length} powder${selected.length > 1 ? "s" : ""} selected`}
              </span>
              <ChevronDown
                size={16}
                  ...
                  >
                    {PRODUCTS.map((p) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border transition ${on ? "border-turmeric bg-turmeric" : "border-forest/25 bg-transparent"}`}
                          >
                            {on && (
                              <svg viewBox="0 0 16 16" fill="none" className="h-3.5 w-3.5">
                                <path
                                  d="M3 8l3.5 3.5L13 5"
                                  stroke="oklch(0.24 0.06 158)"
                                  strokeWidth="2"
                                  strokeLinecap="round"
                          ...
                          <span
                            className={on ? "font-medium text-forest-deep" : "text-forest-deep/75"}
                          >
                            {p.name}
                          </span>
                        </button>
...
function Input({
  ...
  return (
    <label className="block">
      <span className="text-[10px] uppercase tracking-[0.3em] text-forest-deep/70">
        {label}
        {required && " *"}
      </span>
      <input
    ...
    <label className={`block ${className}`}>
      ...
      {showWordCount && (
        <div className="mt-1.5 text-right text-xs text-forest-deep/40">
          {words}
          {maxWords ? `/${maxWords}` : ""} word{words !== 1 ? "s" : ""}
        </div>
      )}
    </label>
  );
